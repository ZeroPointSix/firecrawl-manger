...
> 基于：`docs/MVP/Firecrawl-API-Manager-PRD.md`（v1.1，2026-02-10）
>
> 单一事实来源：本仓库的技术方案以 `docs/agent.md` 为准；API 接口契约见 `docs/MVP/Firecrawl-API-Manager-API-Contract.md`。

## 0. 背景与目标
Firecrawl API Manager（下称 **FCAM**）是一个可容器化部署的轻量级 HTTP 网关服务，用于：
- **集中管理多把 API Key**（Firecrawl / Exa 等多 Provider，加密落库、状态追踪、轮换）
- 对内向多个业务服务提供统一入口：`/api/*`（Firecrawl）、`/exa/*`（Exa）（**鉴权 / 限流 / 配额 / 并发治理**）
- 对运维提供控制面：`/admin/*`（**Key/Client 管理 + 审计日志**，支持 `provider` 维度区分）
...
- `/exa/*`：Exa 原生兼容层（P0：`/exa/search`、`/exa/findSimilar`、`/exa/contents`、`/exa/answer`）
- `/admin/*`：Key/Client CRUD、统计、请求日志、审计日志（支持 `provider` 维度区分）
  - 额外：Keys 批量操作（批量启用/禁用、批量改配额/限流/并发、批量清冷却、批量软删除、批量测试）
- SQLite（单实例）可跑通；生产推荐 Postgres +（可选）Redis

### 1.3 非目标（本阶段不做或可选）
- 完整前端管理台（当前仅提供最小内置 WebUI：`/ui2`；更复杂能力可先用 Swagger/脚本）
- 更多 Provider（ScrapingBee/Apify）——预留接口（Exa 已落地首版）
- Exa Research / Websets 等非 P0 端点
- 多 Provider 智能路由 / 统一搜索协议
...
## 4. 模块与代码层架构（建议目录）
> 目的：让"转发逻辑"和"治理逻辑"可测试、可替换，已扩展为多 Provider（Firecrawl + Exa）。

```text
app/
  main.py                  # FastAPI app 组装（条件注册 exa_compat_router）
  config.py                # 配置加载（env + config.yaml，含 ProviderConfig）
  api/
    data_plane.py          # /api/* 路由
    exa_compat.py          # /exa/* 路由（Exa P0：search/findSimilar/contents/answer）
    control_plane.py       # /admin/* 路由（支持 provider 维度管理）
    deps.py                # 依赖注入（db session, auth, limiter）
  core/
    auth.py                # client/admin 鉴权
    forwarder.py           # httpx 转发、Provider-aware base_url/auth/timeout
    key_pool.py            # Key 选择（provider-scoped 轮询）、冷却、并发 lease
    policy.py              # client 限流/配额/并发校验
    idempotency.py         # X-Idempotency-Key 处理
    redact.py              # 日志/响应脱敏
  db/
    models.py              # 表模型（ApiKey.provider 字段区分 Provider）
    session.py             # 连接与事务
    migrations/            # Alembic
  observability/
...
- 建议字段：
  ...
  - `error_code` + `error_details`（网关侧错误：鉴权/限流/无 key/超时等；需脱敏与截断，避免落敏感数据或超大 payload）
...
### 6.3 网关约束（防 SSRF/滥用）
- 仅允许：`firecrawl.base_url` + 白名单路径（scrape/crawl/search/agent/map/extract/batch）
...
- `server.enable_control_plane`（是否挂载 `/admin/*` 与 `/ui2`；用于端口隔离/仅内网暴露）
...
## 12. 接口契约（前后端/调用方对齐）
- 详细接口清单、请求/响应、错误体、过滤与分页：见 `docs/MVP/Firecrawl-API-Manager-API-Contract.md`。
- 本文档（`docs/agent.md`）只负责“语义定稿 + 一致性与实现边界 + 失败策略”，避免与接口契约重复漂移。
- 路径映射约定：`firecrawl.base_url` 建议配置为上游 root（不包含版本号）。FCAM 按入站路径转发到上游对应版本：`/api/*` 与 `/v1/*` → 上游 `/v1/*`；`/v2/*` → 上游 `/v2/*`（完整示例见接口契约）。
...
错误体结构与示例：见 `docs/MVP/Firecrawl-API-Manager-API-Contract.md`（通用错误体）。
...
### 18.1 文档优先
- 任何接口/语义/失败码/状态机的调整，必须先更新文档：`docs/agent.md`、`docs/MVP/Firecrawl-API-Manager-API-Contract.md`（必要时同步 PRD），再提交代码实现。
- `docs/agent.md` 负责“语义与策略”；`docs/MVP/Firecrawl-API-Manager-API-Contract.md` 负责“接口契约”；禁止在多处重复维护同一事实（避免漂移）。

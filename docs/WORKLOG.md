...
> 更新（2026-02-20）：实施执行入口迁移为 `docs/TODO/README.md`（代办清单）与 `docs/PLAN/README.md`（实施计划）；`docs/project/` 目录不再作为执行入口。
...
**发布内容**
- 额度监控系统完整实现（智能刷新、本地估算、历史追踪、Client 聚合）
- Client 批量管理功能（批量启用、禁用、删除）
- GitHub Actions CI/CD 自动化（代码检查、测试、Docker 构建）
- E2E 测试增强（远程服务器、ClawCloud 部署测试）
- 多项 Bug 修复（额度显示、额度聚合、Docker 认证、测试问题）
- 数据库迁移（4 个新迁移文件）
...
- 数据模型：新增 `BatchAction` 枚举、`BatchClientRequest` 和 `BatchClientResponse` 模型
- 业务逻辑：
  - 支持一次操作最多 100 个 Client
  - 自动去重 client_ids
  - 返回详细的成功/失败统计
  - 使用数据库事务保证原子性
  - 为每个 Client 单独记录审计日志
- 前端 UI：
  - 在 Client 列表中添加复选框（支持单选、多选）
  - 添加批量操作按钮区域（启用、禁用、删除）
  - 显示"已选择 X 个 Client"提示
  - 批量禁用和删除操作需要二次确认
  - 部分失败时显示详细错误信息，保持失败项选中状态
- 测试：新增 17 个集成测试用例，覆盖参数验证、鉴权、批量操作、边界测试、并发测试、事务回滚、审计日志等场景
- 文档：更新 API 契约文档、API 使用指南
...
- `pytest tests/integration/test_batch_clients.py`：通过（17 个测试用例全部通过）
- `npm run type-check`：通过（TypeScript 类型检查无错误）
- `npm run build`：通过（前端构建成功，输出到 app/ui2/）
- 测试覆盖场景：
  - 参数验证（空 IDs、无效 action、超过 100 个）
  - 鉴权测试（无 token、无效 token）
  - 批量启用/禁用/删除成功
  - 部分成功场景（部分 Client 不存在）
  - 全部失败场景（所有 Client 都不存在）
  - 边界测试（单个、100 个、重复 ID）
  - 并发操作测试（数据一致性）
  - 事务回滚测试（数据库错误时回滚）
  - 审计日志测试（记录批量操作）
...
**用户价值**
- 管理员可以一次性启用/禁用/删除多个 Client，提升运维效率
- 部分失败时提供详细的错误信息，便于快速定位问题
- 所有批量操作都记录审计日志，便于追溯和审计
